	...
	if (!*len) /* if nothing left in the buffer, fill it */
	{
		...
		*buf = NULL;
		...
		if (r > 0)
		{
			...
			stripComments(*buf);
			generateHistoryList(info, *buf, info->histcount++);
...
/**
 * retrieveInput - gets a line minus the newline
 */
ssize_t retrieveInput(info_t *info)
{
	...
	if (len) /* we have commands left in the chain buffer */
	{
		...
		validateChain(info, buf, &j, i, len);
		while (j < len) /* iterate to semicolon or end */
		{
			if (checkChain(info, buf, &j))
				break;
			j++;
		}
